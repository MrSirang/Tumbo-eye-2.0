(() => {
  ...
  const API_ROOT = String(window.TUMBO_API_BASE || '').replace(/\/$/, '');
  ...
  async function api(path, options = {}) {
    ...
    try {
      ...
      clearSession();
      showLogin(
        'Cannot reach the Tumbo API. Deploy the .NET backend and set window.TUMBO_API_BASE in admin/config.js.',
      );
      throw new Error('Cannot reach the Tumbo API.');
    }
  ...
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      clearSession();
      showLogin(
        'Could not sign in. Deploy the .NET API and set TUMBO_API_BASE in admin/config.js, then try again.',
      );
    } finally {
